...
from models.NAMattention import Channel_Att
...
def block_decider(block_name,
    ...
    elif block_name == 'RRKA':
        return RRC_block(block_name, in_dim, out_dim, radius, layer_ind, config)
    elif block_name == 'GLCA':
        return GLCA(in_dim, out_dim, config.use_batch_norm, config.batch_norm_momentum,no_relu=True)
...
class GLCA(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, use_bn, bn_momentum, no_relu=False):
        ...
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.mlp1 = nn.Linear(in_dim, out_dim , bias=False)
        self.batch_norm = BatchNormBlock(out_dim, self.use_bn, self.bn_momentum)
        self.dropout = nn.Dropout(p=0.2)
        self.eca_attention=Local_Attention(kernel_size=5)
        if not no_relu:
            self.leaky_relu = nn.LeakyReLU(0.1)
        return
    ...
    def __repr__(self):
        return 'UnaryBlock2(in_feat: {:d}, out_feat: {:d}, BN: {:s}, ReLU: {:s})'.format(self.in_dim,
                                                                                        self.out_dim,
                                                                                        str(self.use_bn),
                                                                                        str( self.no_relu))
...
class Local_Attention(nn.Module):
    ...
    def forward(self, x,batch=None):
        ...
        return (x*y.expand_as(x)).squeeze(2).squeeze(2)
...
class Recurrent_block(nn.Module):
    def __init__(self,block_name, in_dim, out_dim, radius, layer_ind, config,t=4):
        ...
        # nam_attenton
        self.nam_attention=Channel_Att(out_dim // 2).cuda()
    ...
    def forward(self,x,batch):
        ...
        for i in range(self.t):
            if i ==0:
                ...
                x1 = self.nam_attention(x1)
                x1 = x1.squeeze(2).squeeze(2)
                x1=self.KPConv(q_pts, s_pts, neighb_inds,x1)
                x1 = self.leaky_relu(self.batch_norm_conv(x1))
            x1 = x1.unsqueeze(2).unsqueeze(3)
            x1 = self.nam_attention(x1)
            x1 = x1.squeeze(2).squeeze(2)
            x1=self.KPConv(q_pts, s_pts, neighb_inds,x+x1)
            x1 = self.leaky_relu(self.batch_norm_conv(x1))
        x1=self.unary2(x1)
        return x1
...
class RRC_block(nn.Module):
    def __init__(self,block_name, in_dim, out_dim, radius, layer_ind, config,t=4):
        ...
        self.out_dim = out_dim
        self.in_dim = self.out_dim
        self.RC_net1=Recurrent_block(block_name, in_dim, out_dim, radius, layer_ind, config,t=t)
        self.RC_net2 = Recurrent_block(block_name, in_dim, out_dim, radius, layer_ind, config, t=t)
        self.conv1x1=Conv1x1(in_dim,out_dim,bn=True,activation=True).cuda()

    def forward(self,x,batch):
        x = x.unsqueeze(2).unsqueeze(3)
        x=self.conv1x1(x)
        x=x.squeeze(2).squeeze(2)
        res_x=self.RC_net1(x,batch)
        res_x=self.RC_net2(res_x,batch)
        return x+res_x
...
class ResnetBottleneckBlock(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        # nam_attenton
        self.nam_attention=Channel_Att(out_dim // 4).cuda()
    ...
    def forward(self, features, batch):
        ...
        x1 = x1.squeeze(2).squeeze(2)
        # Convolution
        x2 = self.KPConv(q_pts, s_pts, neighb_inds, x1)
        x2 = self.leaky_relu(self.batch_norm_conv(x2))
        # Second upscaling mlp
        x = self.unary2(x2)
        # Shortcut
        if 'strided' in self.block_name:
            shortcut = max_pool(features, neighb_inds)
            # x=max_pool(x,neighb_inds)
        else:
            shortcut = features
